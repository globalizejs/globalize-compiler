...
function versionCheck(runtimeVersion) {
	return "if ((Globalize.runtimeVersion || 0) !== " + JSON.stringify(runtimeVersion) +  ") { throw new Error(\"Runtime version mismatch. Please rebuild the bundle with the correct version of globalize.\"); }";
}
...
function compiler( formattersAndParsers, options ) {
	...
	function extractFormattersAndParsers( object ) {
		JSON.stringify( object, function( key, value ) {
			...
			if ( typeof value === "function" && "runtimeArgs" in value ) {
				var fnRuntimeVersion = value.runtimeVersion || 0;
				if ( runtimeVersion !== undefined && runtimeVersion !== fnRuntimeVersion ) {
					throw new Error( "Formatters and parsers have inconsistent runtime version" );
				}
				runtimeVersion = fnRuntimeVersion;
	...
	code = versionCheck(runtimeVersion) + "\n\n" + code;
